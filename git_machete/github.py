...
class GitHubClient:
    ...
    def derive_open_pull_requests_for_heads(self, heads: List[LocalBranchShortName]) -> List[GitHubPullRequest]:
        heads_query = "+".join(("head:" + head) for head in heads)
        path = f'/search/issues?q=is:pr+state:open+repo:{self.__organization}/{self.__repository}+{heads_query}'
        prs = self.__fire_github_api_request(method='GET', path=path)['items']
        return [GitHubPullRequest.from_json(pr) for pr in prs]

    def derive_open_pull_requests_for_authors(self, authors: List[str]) -> List[GitHubPullRequest]:
        authors_query = "+".join(("author:" + author) for author in authors)
        path = f'/search/issues?q=is:pr+state:open+repo:{self.__organization}/{self.__repository}+{authors_query}'
        prs = self.__fire_github_api_request(method='GET', path=path)['items']
        return [GitHubPullRequest.from_json(pr) for pr in prs]
